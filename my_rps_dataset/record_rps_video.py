...
def get_next_video_number(directory, gesture_label):
    ...
    for filename in existing_files:
        if filename.startswith(f"{gesture_label}_") and filename.endswith(".avi"):
            try:
                number_part = filename.replace(f"{gesture_label}_", "").replace(
                    ".avi", ""
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Record Rock-Paper-Scissors gesture videos."
    )
    parser.add_argument(
        "gesture_label", type=str, help="Gesture label: rock, paper, or scissors"
    )
    ...
    for i in range(num_videos):
        ...
        print(
            f"Get ready! Recording '{gesture_label}' video {video_number} ({i+1}/{num_videos})"
        )
        time.sleep(2)

        out = cv2.VideoWriter(
            f"{save_dir}/{gesture_label}_{video_number}.avi",
